...
transformations. The live dependency graph, used for deadlock checks, is transient.
...
[shared operations](runtime.md) for the exact lifetime and race rules.
...
path. [Native PTC primitives](runtime.md) explains why each remains and
...
## 9. Architecture decisions after adversarial review

A skill describes a procedure; a request chooses an application of it; an execution
owns a fresh context; an artifact is an immutable published observation. These are
separate identities. A node behaves like an asynchronous transformation, but it is
not assumed to be a mathematical pure function: model sampling, external reads and
side effects can vary. Sharing is therefore explicit and exact, never inferred from
similar prose. Read [runtime.md](runtime.md) for the complete implemented contract.

Keep caller-local `key` separate from work equivalence. The same caller/key is a
retry of one selected operation, including failure; a new key permits a new attempt.
`reuse: fresh` avoids joining other operations, but does not override an existing
key binding. `reuse: session` opts into sharing matching work. Removing both fields
would conflate transport retry, intentional repetition and common-subexpression
reuse. A persona or random input is not an adequate substitute for an explicit call
lifetime contract. No identity-only migration is adopted by this revision.

A replacement artifact is a new value. If b is insufficient, call b with a different
task and appropriate explicit evidence, or run a different transformation. If b is
incorrect, publish a correction and pass that new ref to affected downstream calls.
Do not silently remap an old request to a new answer obtained with different inputs.
A review of b can recommend a repair without granting permission to mutate another
caller's result or the session cache. The core does not parse verdicts for this purpose.

Automatic dependency invalidation would need explicit cache/version semantics,
authorized writers, a sufficiently complete dependency index and a rule for consumers
already running when an upstream version changes. The current provenance is useful
evidence but not that complete index. See [evolution and memory](evolution-and-memory.md)
for acceptance conditions before adding persistent or cross-session behavior.

The native PTC surface stays at eight host endpoints. Review, red teaming, coherence,
repair, racing, retries and joins are ordinary code over those endpoints. No new
frontmatter or node attributes are added. A different agent implementation belongs
in host executor registration and bindings. Executor configuration must stay stable
once a session seals; closures hiding changing model, credential or data state can
break sharing assumptions even when a request's JSON is unchanged.

Fresh contexts exclude inherited conversation and private interpreter state. They
do not prove independent judgments, remove model bias or make an audit trustworthy.
A product gate should be enforced by trusted application code over the exact candidate
and a defined schema; a model-written request to review is only orchestration.

## 10. Continuation contract

When resuming this project, read this file, [runtime.md](runtime.md),
[patterns.md](patterns.md), [evolution-and-memory.md](evolution-and-memory.md), and
[review-findings.md](review-findings.md). Treat checked-in code and executed tests as
the source of implemented guarantees. The repository's historical proposal is
preserved for rationale, clearly marked as non-normative.

Keep Deep Agents as the current agent adapter unless a separate change is justified.
Do not introduce mandatory orchestration scripts into prose skills. The arithmetic
resource is an explicit deterministic utility; offline fixture scripts are test
inputs, not the live authoring model. Preserve the two prompts and actual exported
PTC trajectories, and regenerate them when execution behavior changes.

Future changes should answer: what host-owned effect is missing, why ordinary
composition cannot express it safely, which authority it confers, how cancellation
and retry behave, and which adversarial test proves the contract. Convenience alone
does not justify another primitive or another attribute on every node.
