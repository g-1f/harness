# Historical architecture proposal — not the implemented contract

Preserved from `new_doc.md` at commit `a5764c2`. This draft mixes proposals with
unverified implementation claims, including commits absent from the fetched remote.
Its API snippets and verification counts must not be used as current instructions.
See [the review](../review-findings.md), [design](../design.md) and
[runtime](../runtime.md) for reconciled decisions and verified behavior.
Relative links in the preserved text below retain their historical spelling.
