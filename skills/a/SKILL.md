...
A parallel capacity investigation may already be using that same execution. Open
the shared operation, observe its accepted measurement checkpoint if published,
then await its final result. A checkpoint can inform this interpretation while b
continues; record both checkpoint and final refs when consumed.
